...
MCP-сервер с инструментами для поиска в интернете и загрузки содержимого веб-страниц. Использует multi-engine поиск через Playwright (Yandex → Google → Bing → Rambler) и извлекает текст страниц в markdown-формате.

## Инструменты

| Инструмент | Описание | Аргументы |
|------------|----------|-----------|
| `web_search` | Поиск в интернете через цепочку поисковиков (Google → Bing → Yandex → Rambler, до 5 результатов: заголовок, URL, сниппет) | `query` (str) — текст запроса |
